...
```{sql sqlite-sql-agg-select, connection=con, tab.cap="SQL select 쿼리문"}
...
```{sql sqlite-sql-agg-min, connection=con, tab.cap="최소값 집합 함수 적용 쿼리문"}
...
```{sql sqlite-sql-agg-max, connection=con, tab.cap="최대값 집합 함수 적용 쿼리문"}
...
```{sql sqlite-sql-agg-avg, connection=con, tab.cap="평균값 집합 함수 적용 쿼리문"}
select avg(reading) from Survey where quant='sal';
```

```{sql sqlite-sql-agg-count, connection=con, tab.cap="개수 집합 함수 적용 쿼리문"}
select count(reading) from Survey where quant='sal';
```


```{sql sqlite-sql-agg-sum, connection=con, tab.cap="합계 집합 함수 적용 쿼리문"}
...
```{sql sqlite-sql-agg-minmax, connection=con, tab.cap="최소, 최대값 집합 함수 적용 쿼리문"}
...
```{sql sqlite-sql-agg-composition, connection=con, tab.cap="원 결과값과 집합 함수를 적용한 쿼리문"}
...
```{sql sqlite-sql-agg-null, connection=con, tab.cap="NULL 값이 포함된 원데이터에 집합 함수를 적용한 쿼리문"}
select person, count(*) from Survey where quant='sal' and reading<=1.0;
```
...
```{sql sqlite-sql-agg-null-deal, connection=con, tab.cap="NULL 값이 포함된 원데이터를 명시적으로 처리한 후 집합 함수를 적용한 쿼리문"}
select min(dated) from Visited;
```

명시적으로 항상 다음과 같이 필터하는 쿼리를 작성할 필요가 없다.

```{sql sqlite-sql-agg-null-explicit, connection=con, tab.cap="NULL 값이 포함된 원데이터를 명시적으로 처리하지 않는 집합 함수를 적용한 쿼리문"}
...
```{sql sqlite-sql-agg-rad-issue, connection=con, tab.cap="주의깊이 살펴볼 쿼리문"}
...
```{sql sqlite-sql-agg-rad, connection=con, tab.cap="사람별로 작성한 쿼리문 문제 예제"}
select person, count(reading), round(avg(reading), 2)
from  Survey
where quant='rad'
and   person='dyer';
...
```{sql sqlite-sql-agg-group-by, connection=con, tab.cap="`group by` 문을 사용해서 사람별로 작성한 쿼리문 예제"}
select   person, count(reading), round(avg(reading), 2)
from     Survey
where    quant='rad'
group by person;
...
```{sql sqlite-sql-agg-groupby-advanced, connection=con, tab.cap="`group by` 문을 확장하여 적용한 쿼리문 사례"}
...
```{sql sqlite-sql-agg-groupby-advanced2, connection=con, tab.cap="사람별로 측정값을 정렬한 쿼리문 사례"}
select   person, quant, count(reading), round(avg(reading), 2)
from     Survey
