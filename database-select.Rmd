...
```{sql db-selection, connection=con, tab.cap="첫번째 SQL 쿼리"}
...
```{sql db-selection-variation, connection=con, tab.cap="SQL 쿼리문은 대소문자 구분하지 않음"}
SeLeCt FaMiLy, PeRsOnAl FrOm PeRsOn;
```
...
```{sql db-selection-order, connection=con, tab.cap="칼럼 교환"}
...
```{sql db-selection-dup, connection=con, tab.cap="칼럼명 중복 선택"}
...
```{sql db-selection-wildcard, connection=con, tab.cap="모든 칼럼 선택"}
select * from Person;
```
