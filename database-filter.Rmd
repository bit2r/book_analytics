...
```{sql sqlite-sql-filter-site, connection=con, tab.cap="SQL 필터 쿼리문"}
...
```{sql sqlite-sql-filter-select, connection=con, tab.cap="SQL 칼럼 선택과 결합된 필터 쿼리문"}
...
```{sql sqlite-sql-filter-boolean, connection=con, tab.cap="SQL 필터 부울연산 반영쿼리문"}
...
```{sql sqlite-sql-filter, connection=con, tab.cap="SQL 필터 부울 선택(OR) 연산자 반영쿼리문"}
...
```{sql sqlite-sql-filter-in, connection=con, tab.cap="SQL 필터 가독성 높은 부울 선택(OR) 연산 쿼리문"}
...
```{sql sqlite-sql-filter-order, connection=con, tab.cap="SQL 필터 부울 선택(OR) 연산 적용순서"}
...
```{sql sqlite-sql-filter-order-parenthesis, connection=con, tab.cap="SQL 필터 괄호적용 부울 선택(OR) 연산 적용순서"}
select * from Survey where quant='sal' and (person='lake' or person='roe');
```
...
```{sql sqlite-sql-filter-order-distinct, connection=con, tab.cap="SQL 중복제거 선택문과 결합된 필터 부울 선택(OR) 연산"}
