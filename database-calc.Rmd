...
```{sql sqlite-sql-calc-reading, connection=con, tab.cap="신규 칼럼 생성 쿼리문"}
...
```{sql sqlite-sql-calc-rounding, connection=con, tab.cap="신규 칼럼 반올림 적용 쿼리문"}
...
```{sql sqlite-sql-calc-concat, connection=con, tab.cap="접합연산자 적용 신규 칼럼 생성 쿼리문"}
...
```{sql sqlite-sql-calc-union, connection=con, tab.cap="union 연산자 적용 쿼리 두개 결합 결과"}
...
```{sql sqlite-sql-calc-visited, connection=con, tab.cap="", eval = FALSE}
